/******************************************************************************/
/*                                                                            */
/*  Programmname  : GSEMerge.c                                                */
/*  Verwendung    : Zusammenfuehren der GPS-, SIG- und EFF-Dateien            */
/*  Bibliotheken  :                                                           */
/*  Autor         : Geo-X, JD                                                 */
/*  Compiler      : WATCOM C/C++ v. 10.0                                      */
/*  System        : Standard C                                                */
/*  Erstellung    : 25 Sep 2003                                               */
/*  énderung      : 29 Sep 2003 - Pruefung auf mehrfach SIG's im 4sek-Interv. */
/*                                EFF's werden immer an SIG's angehaengt      */
/*                  11 Nov 2003 - Spalten SIG_17 und SIG_19 entfallen         */
/*                                Anfuehrungszeichen bei Ausgabe entfernt     */
/*                                Dateien fangen nicht immer bei 00 an und es */
/*                                koennen Spruenge vorkommen                  */
/*                                EFF-Zeile immer ausgeben                    */
/*                  18 Nov 2003 - SonderfÑlle: Leeres EFF Feld 11 und , in 13 */
/*                                Bei EFF Feld 3: C, R, E werden die Werte    */
/*                                ab Feld 5 beibehalten                       */
/*                   9 Dez 2003 - Wenn Feld 3 ein A ist, nur das Feld mit     */
/*                                neuem Inhalt Åbernehmen, sonst beibehalten  */
/*                  12 Dez 2003 - Neue Spaltenbezeichnungen  (UK)             */
/*                  31 Jan 2004 - EFF Spalten Track und Leg mit rausschreiben */
/*                                !!! - Zeitzuordnun funktioniert noch nicht  */
/*                                !!! noch nicht verwenden EFF_Track und      */
/*                                !!! EFF_Leg (UK)                            */
/*                   1 Feb 2004 - Distanzberechnung -> extra Spalte nach      */
/*                                Lat und Lon (UK)                            */
/*                  21 Feb 2004 - JD: EFFdata[0/1] wurden falsch ausgegeben   */
/*                                Distanz in km umgerechnet                   */
/*                                GSE wird erst geschrieben, wenn EFF B gef.  */
/*                                EFF-Zeilen C werden ignoriert               */
/*                  25 Mar 2004 - JD: Wenn E/C, Stop der GSE-Ausgabe bis B/R  */
/*                  30 Mar 2004 - JD: Kommas in SIG werden durch ; ersetzt    */
/*                  31 Mar 2004 - JD: E/C - Stop Ausgabe korrigiert           */
/*                  13 Mai 2004 - JD: Bis Ende EFF Daten schreiben            */
/*                   4 Jun 2004 - JD: Fehlerhafer Umbruch bei SIG-Ende        */
/*                                Distanz beginnt bei neuen Abschnitten mit 0 */
/*                                                                            */
/*                  28 Apr 2005 - Phil Lovell: Added AUDIO_VOR section after  */
/*                                change in file name convention (an extra    */
/*                                letter at the beginning specifies the       */
/*                                computer used to create the files, and the  */
/*                                final two-digit number 00-99 has been       */
/*                                replaced with a single letter a-z).  The    */
/*                                AUDIO_VOR version also assumes that there   */
/*                                will be no comma in the "subjective" effort */
/*                                field (i.e. it will be "mg", not "m,g") and */
/*                                splits the two letters into separate        */
/*                                columns in the output file.                 */
/*                                Change the line below to                    */
/*                                #define AUDIO_VOR 0                         */
/*                                to make the version                         */
/*                                that reads the old-style files              */
/******************************************************************************/
...
/* Als Argument muss der Filename Åbergeben werden */
...
double lat2, lon2, dist, dlat, dlon, dx, dy;
double sm = 1855.397859;    /* Definition 1 Seemeile */
/* UK Distanz-Berechnung 31.01.04 */

    /* --------------------  UK  ------ */
    /* Test Distanzberechnung einbauen  */
    /* UK 04                            */

    /* 4 Jun 2004 JD: Bei B oder R Distanz neu bei 0 anfangen lassen */
    if((id == 0) || (EFFdata[2][0] == 'B') || (EFFdata[2][0] == 'R')){
        ...
        dlat = lat1 - lat2;
        dlon = lon1 - lon2;
        dx = dlon * 60 * sm * cos((lat2 * 3.14159265) / 180);
        dy = dlat * 60 * sm;
        dist = sqrt(dx * dx + dy * dy);
        dist /= 1000;   /* 21 Feb 2004 JD: Distanzumrechnung in km */
        lat1 = lat2;
        lon1 = lon2;
    }
...
int schreibeEFF(void)
{
    ...
    fprintf(fpGSE, "\n");
    /* Beim Schreiben einer SIG-Ausgabe wird immer eine EFF-Ausgabe der letzten Daten angehÑngt */
    /* FÅr diesen Fall wird eine Kopie benîtigt */
...
int  n, m;
    ...
    /* 30 MÑr 2004 - JD: Die , zwischen den "" durch ; ersetzen */
    p = zeile;
    n = 0; /* Zaehler fÅr " */
...
#else
		/* Old VOR */
        /* 18 Nov 2003 - Sonderfall: Wenn Feld 11 leer ist muss ein leeres Feld 12 angehÑngt werden */
        if ((n == 10) && (m == 0)) {
            n++;
            EFFdata[n][m] = 0;
        } /* endif */
...
for (n = 0; n < 13; ++n)
    ...
    /* 9 Dez 2003 - Wenn Feld 3 ein A ist, nur das Feld mit neuem Inhalt Åbernehmen, alle anderen beibehalten */
    if (EFFdata[2][0] == 'A') {
        for (n = 4; n < 13; n++) {
            if (EFFdata[n][0] == 0) {
...
int main(int argc, char *argv[])
{
    ...
    /* Filename aus Argument kopieren und nach 6. Stelle abschneiden */
    ...
    /* 11 Nov 2003 - Dateinamen beginnen nicht unbedingt mit Nummer 00, auch bis 99 versuchen zu oeffnen   */
    do {
        sprintf(SIGfile, "%s" SIG_EFF_FORMAT ".SIG", filename, SIG_EFF_SUFFIX(SIGfilecntr));  /* Name aus filename und lfd. Nummer bauen */
        fpSIG = fopen(SIGfile, "r");                            /* Versuche Datei zu oeffnen               */
    ...
    /* Kopf-Zeile */ /* UK 12.12.03 */
    fprintf(fpGSE, "Nr\tGPS_Zeit\tLat\tLon\tDistance\tHeight" /* 31.01.04 UK - Spalte Distanz ergaenzt*/
                   ...
                   "\tEFF_Track\tEFF_Leg" /* 31.01.04 UK soll ebenfalls rausgeschrieben werden*/
